...
export class MedicalRecordService {
  ...
  async getByPet(petId) {
    ...
    return items.filter(item => item.pet === petId).map(item => new MedicalRecord(item));
  }
  async create(record) { return new MedicalRecord(await apiRequest(this.baseUrl, { method: 'POST', body: JSON.stringify(record.toApi()) })); }
